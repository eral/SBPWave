...
namespace eral.SBPWave.Test.BuiltinCompatibleTest {
	...
	public class PrefabsTest {
		...
		private const string kAssetsBasePath = "Assets/SBPWaveTests/BuiltinCompatibleTest/Runtime/Prefabs";
		private const string kAssetBundlesPath = "Assets/SBPWaveTests/AssetBundles~/Prefabs_";
		private readonly string[] kAssetNames = new[]{"Top", "Value"};
		private readonly string[] kAssetBundleNames = new[]{"prefabs_top", "prefabs_value"};
		...
		private void CreateAssetBundles(TestUtility.Style style) {
			...
			var builds = new[]{
				new AssetBundleBuild{
					...
					assetNames=new[]{
						$"{kAssetsBasePath}/{kAssetNames[0]}.prefab"
					},
				...
				new AssetBundleBuild{
					...
					assetNames=new[]{
						$"{kAssetsBasePath}/{kAssetBundleVariants[0]}/{kAssetNames[1]}.prefab"
					},
					addressableNames=null
				},
				new AssetBundleBuild{
					...
					assetNames=new[]{
						$"{kAssetsBasePath}/{kAssetBundleVariants[1]}/{kAssetNames[1]}.prefab"
					},
					addressableNames=null
				},
			};
		...
		private IEnumerator TestNormal(TestUtility.Style style) {
			yield return LoadAndTest(style, kAssetBundleVariants[0], (script, ab)=>{
				Assert.True(script != null);
				Assert.AreEqual(kAssetBundleVariantValues[0], script.Value);
		...
		private IEnumerator TestVariant(TestUtility.Style style) {
			yield return LoadAndTest(style, kAssetBundleVariants[1], (script, ab)=>{
				Assert.True(script != null);
				Assert.AreEqual(kAssetBundleVariantValues[1], script.Value);
		...
		private IEnumerator TestLoadAssetDirectFromVariant(TestUtility.Style style) {
			...
			for (var i = 0; i < kAssetBundleVariants.Length; i++) {
				...
				var abReq = ab.LoadAssetAsync<GameObject>(kAssetNames[1]);
				while (!abReq.isDone) yield return null;
				var prefab = (GameObject)abReq.asset;
				{
					Assert.AreEqual(kAssetNames[1], prefab.name);
					var govalue = Object.Instantiate(prefab);
					var value = govalue.GetComponentInChildren<PrefabsValue>();
					Assert.AreEqual(kAssetBundleVariantValues[i], value.Value);
					Object.DestroyImmediate(govalue);
				}
				ab.Unload(true);
			}
		}

		private IEnumerator LoadAndTest(TestUtility.Style style, string variant, System.Action<PrefabsValue, AssetBundle> test) {
			var assetBundlesPath = TestUtility.AddStyleStringToEnd(style, kAssetBundlesPath);
			...
			var ab = abcReq.assetBundle;
			var abReq = ab.LoadAssetAsync<GameObject>(kAssetNames[0]);
			while (!abReq.isDone) yield return null;
			var prefab = (GameObject)abReq.asset;
			{
				Assert.AreEqual(kAssetNames[0], prefab.name);
				var gotop = Object.Instantiate(prefab);
				var top = gotop.GetComponentInChildren<PrefabsTop>();
				var govalue = default(GameObject);
				var value = default(PrefabsValue);
				if (top.Value != null) {
					govalue = Object.Instantiate(top.Value);
					value = govalue.GetComponentInChildren<PrefabsValue>();
				}
				test(value, ab2);
				if (govalue != null) {
					Object.DestroyImmediate(govalue);
				}
				Object.DestroyImmediate(gotop);
			}
			ab.Unload(true);
			ab2?.Unload(true);
		}
